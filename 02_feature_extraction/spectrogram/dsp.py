...
MODULE_PATH = os.path.join(os.path.dirname(os.path.realpath(
    __file__)), 'third_party', 'speechpy', '__init__.py')
...
def generate_features(implementation_version, draw_graphs, raw_data, axes, sampling_freq,
    ...
    for ax in range(0, len(axes)):
        ...
        if implementation_version < 3:
            power_spectrum = (power_spectrum - np.min(power_spectrum)) / \
                (np.max(power_spectrum) - np.min(power_spectrum))
            ...
            power_spectrum = (power_spectrum - noise_floor_db) / \
                ((-1 * noise_floor_db) + 12)
        ...
        if draw_graphs:
            ...
            if not show_axes:
                cax = ax.imshow(
                    power_spectrum, interpolation='nearest', cmap=cm.coolwarm, origin='lower')
                ...
                plt.yticks(np.linspace(0, height, 15), [
                           math.ceil(x) for x in freqs])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Spectrogram from sensor data')
    ...
    parser.add_argument('--show-axes', type=lambda x: (str(x).lower() in ['true', '1', 'yes']), required=False,
                        help='Whether to show axes on the graph')

    args = parser.parse_args()

    raw_features = np.array([float(item.strip())
